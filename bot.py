...
# 🟢 NEW API ENDPOINT (Hugging Face)
API_BASE_URL = "https://chirag930-secretchiragxnx.hf.space" 
...
# --- KEEP-ALIVE LOGIC (Hugging Face ko jagane ke liye) ---
def keep_alive_space():
    while True:
        try:
            requests.get(API_BASE_URL, timeout=10)
            print("[SYSTEM] Ping sent to Hugging Face to stay awake.")
        except:
            pass
        time.sleep(600) # Har 10 minute mein ping karega
...
def inbox_monitor_worker(chat_id, email):
    ...
    while chat_id in user_sessions and user_sessions[chat_id]['email'] == email:
        try:
            # Hugging Face API call with increased timeout
            resp = requests.get(f"{API_BASE_URL}/inbox", params={"email": email}, timeout=20)
        ...
        time.sleep(10) # 10 seconds gap for stable connection on HF
...
def handle_callbacks(call):
    ...
    if call.data == "gen_new":
        ...
        try:
            res = requests.get(f"{API_BASE_URL}/generate", timeout=15).json()
            email = res['email']
...
if __name__ == "__main__":
    print(f"[SYSTEM] Bot by {DEV_USERNAME} is starting on Hugging Face Backend...")
    # Start Keep-Alive Thread
    threading.Thread(target=keep_alive_space, daemon=True).start()
